...
update_docker() {
    ...
    local docker_exit=0
    docker run --rm -v /var/run/docker.sock:/var/run/docker.sock containrrr/watchtower --run-once || docker_exit=$?
    ...
    if [[ $docker_exit -eq 0 ]]; then
        ...
        echo -e "${YELLOW}  ! Some containers were not updated${RESET}"
    fi
